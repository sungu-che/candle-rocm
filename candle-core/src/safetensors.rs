...
fn convert_slice<T: WithDType>(data: &[u8], shape: &[usize], device: &Device) -> Result<Tensor> {
    ...
    if (data.as_ptr() as usize).is_multiple_of(size_in_bytes) {
        // Fast path: ROCm direct upload — mirrors hipfire's mmap→upload_raw→hipMemcpy.
        if let Device::Rocm(inner) = device {
            let storage = inner
                .storage_from_bytes(data, T::DTYPE)
        ...
        // Misaligned: ROCm can still upload raw bytes directly (alignment handled by hipMemcpy).
        if let Device::Rocm(inner) = device {
            let storage = inner
                .storage_from_bytes(data, T::DTYPE)
                .map_err(|e| Error::Msg(e.to_string()))?;
        ...
        // SAFETY: We just created c, so the allocated memory is necessary
...
fn convert_<T: WithDType>(view: &st::TensorView<'_>, device: &Device) -> Result<Tensor> {
    ...
    // Fast path: ROCm direct upload — mmap bytes → hipMemcpy, no CpuStorage staging.
    // Mirrors hipfire's `tensor_data()` → `upload_raw()` → `hipMemcpy` pattern.
    if let Device::Rocm(inner) = device {
        let elem_size = T::DTYPE.size_in_bytes();
        let aligned_ptr = (data.as_ptr() as usize).is_multiple_of(elem_size);
        if aligned_ptr {
            // Aligned: upload directly, no copy.
            let storage = inner
                .storage_from_bytes(data, T::DTYPE)
                .map_err(|e| Error::Msg(e.to_string()))?;
            let storage = Storage::Rocm(storage);
            return Ok(from_storage(storage, shape, BackpropOp::none(), false));
        }
    }
    // Fallback: misaligned or non-ROCm — go through convert_slice (may copy).
    convert_slice::<T>(data, &shape, device)
}
        ...
        match dtype {
            ...
            DType::F6E2M3 | DType::F6E3M2 | DType::F4 | DType::F8E8M0 => {
                ...
                let storage = match device {
                    ...
                    Device::Rocm(inner) => {
                        let storage = inner
                            .storage_from_bytes(data, dtype)
                            .map_err(|e| Error::Msg(e.to_string()))?;
                        Storage::Rocm(storage)
                    }
                    #[cfg(not(feature = "rocm"))]
                    Device::Rocm(_) => {
                        return Err(Error::Msg("ROCm support not compiled".to_string()));
                    }
...
fn convert_dummy(view: &st::TensorView<'_>, device: &Device) -> Result<Tensor> {
    ...
    let storage = match device {
        ...
        #[cfg(feature = "rocm")]
        Device::Rocm(inner) => {
            // Fast path: mirrors hipfire's mmap→upload_raw→hipMemcpy, no Vec copy.
            let storage = inner
                .storage_from_bytes(data, dtype)
                .map_err(|e| Error::Msg(e.to_string()))?;
            Storage::Rocm(storage)
...
impl MmapedSafetensors {
    ...
    /// Advise the kernel to release mmap pages with `MADV_DONTNEED`.
    ///
    /// After all tensors have been loaded (copied) to their target device
    /// (typically GPU), the mmap backing pages are no longer needed for the
    /// duration of inference. Calling this tells the kernel it can reclaim
    /// those physical pages immediately, rather than waiting for memory
    /// pressure to evict them.
    ///
    /// **Safety:** The mmap regions must not be accessed after this call.
    /// If a tensor is loaded again (e.g. after a model reload), the pages
    /// will be faulted back in from disk, which is correct but slow. Only
    /// call this when you are certain the mmap data is no longer needed.
    ///
    /// Returns the total number of bytes advised (for logging).
    pub fn advise_dontneed(&self) -> usize {
        use memmap2::UncheckedAdvice;
        let mut total = 0usize;
        for st in &self.safetensors {
            let mmap: &memmap2::Mmap = st.backing_cart();
            let len = mmap.len();
            // SAFETY: MADV_DONTNEED is safe on read-only mmap'd files.
            // Pages will be re-faulted from disk if accessed again.
            if unsafe { mmap.unchecked_advise(UncheckedAdvice::DontNeed) }.is_err() {
                // Best-effort: ignore failures (e.g. NFS-backed mmap or
                // exotic platforms). This is an optimisation hint, not a
