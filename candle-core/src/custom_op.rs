...
pub trait CustomOp1 {
    ...
    /// The forward pass, as run on a ROCm (AMD GPU) device.
    /// Default: error (no ROCm implementation).
    fn rocm_fwd(
        &self,
        _storage: &crate::RocmStorage,
        _layout: &Layout,
    ) -> Result<(crate::RocmStorage, Shape)> {
        Err(crate::Error::Msg(
            format!("no rocm implementation for {}", self.name()),
        ))
    }
